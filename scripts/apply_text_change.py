...
if env_file.exists():
    ...
    # debug to stderr (won't break stdout JSON)
...
if not AZ_ENDPOINT or not AZ_KEY or not AZ_DEPLOY:
    print(
        "Error: Set AZURE_OPENAI_ENDPOINT, AZURE_OPENAI_KEY, AZURE_OPENAI_DEPLOYMENT in environment."
    )
    sys.exit(1)
...
def summarize_design(d: dict) -> str:
    ...
    if d.get("color_palette"):
        parts.append("Colors: " + ", ".join(d["color_palette"]))
    if d.get("fabrics"):
        parts.append("Fabrics: " + ", ".join(d["fabrics"]))
    if d.get("prints_patterns"):
        parts.append("Prints: " + ", ".join(d["prints_patterns"]))
    if d.get("garment_type"):
        parts.append(f"Garment: {d['garment_type']}")
    if d.get("silhouette"):
        parts.append(f"Silhouette: {d['silhouette']}")
    if d.get("sleeves"):
        parts.append(f"Sleeves: {d['sleeves']}")
    if d.get("neckline"):
        parts.append(f"Neckline: {d['neckline']}")
    if d.get("length"):
        parts.append(f"Length: {d['length']}")
    if d.get("style_fit"):
        if isinstance(d["style_fit"], str):
            parts.append("Style/Fit: " + d["style_fit"])
        elif isinstance(d["style_fit"], list):
            parts.append("Style/Fit: " + ", ".join(d["style_fit"]))
    if d.get("trims_and_details"):
        parts.append("Trims: " + ", ".join(d["trims_and_details"]))
    return " ".join(parts)
...
def apply_change(base_design: dict, user_change: str, temperature=0.0):
    ...
    if parsed is None:
        print(
            "WARNING: GPT did not return parseable JSON. Returning modified base with provenance."
        )
        base_copy = dict(base_design)
        base_copy["provenance"] = (
            f"failed_to_parse_gpt_output; user_change={user_change}"
        )
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print(
            'Usage: python apply_text_change.py path/to/base.design.json "change text..." OR pass a change file path',
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    # If change_arg is a path to a file, read its contents; otherwise use as text
    change_path = Path(change_arg)
    if change_path.exists():
        change_text = change_path.read_text(encoding="utf-8")
    else:
        change_text = change_arg

    if not base_path.exists():
        print("Base design file not found:", base_path, file=sys.stderr)
        sys.exit(1)
